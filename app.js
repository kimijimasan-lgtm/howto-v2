...
function renderEditor(container) {
  ...
  if (!dbgEl) {
    ...
    dbgEl.id = '_dbg';
    dbgEl.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:999999;background:rgba(0,0,0,0.82);color:#0f0;font:11px/1.4 monospace;padding:6px 8px;max-height:180px;overflow-y:auto;pointer-events:none;';
    document.body.appendChild(dbgEl);
  }
  const dbg = (label, obj) => {
    const line = document.createElement('div');
    line.textContent = `[${label}] ` + Object.entries(obj).map(([k,v]) => `${k}=${typeof v==='number'?v.toFixed(1):v}`).join(' | ');
    dbgEl.insertBefore(line, dbgEl.firstChild);
    while (dbgEl.children.length > 12) dbgEl.removeChild(dbgEl.lastChild);
  };

  // ① タップ後のフォーカス時: 最小スクロール（nearest）でカーソルを画面内に収める
  tiptapEditor.on('focus', () => {
    const scrollYAtFocus = window.scrollY;
    const edContent = document.getElementById('edContent');
    dbg('focus', { scrollY: scrollYAtFocus, edTop: edContent ? edContent.scrollTop : -1 });
    setTimeout(() => {
      ...
      dbg('focus+400ms', {
        ...
        edTop: edC ? edC.scrollTop : -1,
        elTop: rect ? rect.top : -1,
        elBottom: rect ? rect.bottom : -1,
        vvH: window.visualViewport ? window.visualViewport.height : -1
      });
  ...
  if (window.visualViewport) {
    ...
    const onVVResize = () => {
      ...
      vvTimer = setTimeout(() => {
        ...
        dbg('vvResize', { scrollY: window.scrollY, edTop: edContent.scrollTop, vvH: vv.height });
        ...
        requestAnimationFrame(() => {
          ...
          const pad = 40;
          dbg('vvResize+rAF', { scrollY: window.scrollY, edTop: edContent.scrollTop, cursorBot: coords.bottom, vvH: vv.height });
