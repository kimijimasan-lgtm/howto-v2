...
- **PC活用促進バナーの文字サイズ・コントラスト改善（完了・本番デプロイ済み、style.css?v=724）**: ホーム画面下部のバナー（`renderPcPromoBanner()`、折りたたみ時の小さいバッジ／タップ後展開する大きいカードの2状態）について、「枠のサイズに対して文字が小さすぎてバランスが悪い」との指摘を受け修正。
  - **小さいバッジ**（`.pc-promo-small-line1/2`）: フォントサイズ0.72rem/0.68rem→0.78rem/0.74rem。文字色をline1=白・line2=明るいゴールド(`#fde68a`)に変更（濃い紫背景`#4c1d6e`に対しコントラスト比 約10〜12:1、WCAG AAA基準）。padding調整（0.25rem 0.7rem→0.2rem 0.5rem）
  - **大きいカード**（`.pc-promo-card-title/body`）: タイトル1.05rem→1.4rem、本文0.85rem→1.05rem。同様に白＋明るいゴールドの配色に変更、gap・paddingも再調整
  - **サイズ決定の根拠**: このバナーの幅は左下FAB（ゲストは「100均」ボタン、幅約83px。ログイン済みユーザーはファイルアイコン、幅44pxで実はより余裕がある）と右下検索FABの隙間に動的にフィットする（`positionPcPromoBanner`/`positionPcPromoBannerLarge`、`app.js`）ため、単純に大きくすると狭い画面幅でテキストが`overflow:hidden`により欠ける危険があった。**ゲスト×iPhone SE幅(375px)という最悪ケースでの実測**（Noto Sans JPをcanvasで計測）に基づき、小さいバッジのline1は170px前後が上限と判明したため0.78rem（実測166px、隙間174.6px中マージン約8px）に設定。それ以上（0.9rem台など）は同条件で確実に欠けることを実機幅シミュレーションで確認済み
  - **検証方法**: 開発サンドボックスの仮想ディスプレイが942×589px固定でウィンドウリサイズが効かない制約があったため、`#pcPromoBanner`要素に一時的なインラインstyle（width固定・メディアクエリのdisplay:none上書き）を注入してモバイル相当の実測幅を再現。折りたたみ時バッジ（最悪ケース幅193.8px）・展開時カード（幅300px、iPhone SE相当）の両方で文字欠け・閉じるボタンとの衝突がないことをスクリーンショットで確認
  - **未実施**: 実機（実際のiPhone）での最終見た目確認はユーザー対応待ち
- **PC活用促進バナーの続報修正・タイトル折り返し解消＋配色を紫→ターコイズに変更（完了・本番デプロイ済み、style.css?v=725）**: 上記の対応後、実機で①大きいカードのタイトル「PCでも同じ画面が見られます」が2行に折り返される②背景の紫が解説パネル等の既存の紫系パネルに埋もれる、の2点を指摘され再修正。
  - **①折り返し解消**: `.pc-promo-card-title`に`white-space: nowrap`を追加し、暴走防止のためフォントサイズを1.4rem→1.2remに縮小、横paddingも1.8rem→1.4remに縮小（`.pc-promo-card`）して枠内の使用可能幅を拡大。この大きいカードの幅は左右FABの**内端どうし**（`leftFab.left`〜`rightFab.right`＝`innerWidth-32px`、FAB自体の幅に依存しない）で決まるため、最も狭いiPhone SE幅(375px)でも幅343px・padding差引後content幅298.2px。「💻 PCでも同じ画面が見られます」の実測幅（Noto Sans JP 800・canvas計測）は1.2remで286.5px、マージン約12pxで一行固定を保証
  - **②配色変更**: 大きいカード・小さいバッジとも背景を`#4c1d6e`（紫）→`#115e59`（濃いターコイズ）に変更。ゴールドの枠線(`#fbbf24`)は維持し、テキストは白（コントラスト比約7.6:1）＋補助テキストに明るいゴールド`#fde68a`（同約6:1）でAA基準を満たしつつ、緑・ピンク・青などの既存パネル色から明確に区別できる配色に
  - **検証時の注意点**: ローカルサーバー(`python -m http.server 8080`)でCSS編集後にキャッシュバスティング番号を上げても、ブラウザ側に古い`style.css`がキャッシュされ反映されないことがあった（Service Workerではなく通常のHTTPキャッシュ。`?nocache=`のようなクエリ付きURLで再読み込みして解消）。**ローカル検証でCSS変更が反映されないときはこの可能性をまず疑うこと**
  - **検証**: 実機幅の最悪ケース（ゲストの「100均」ボタン＋iPhone SE幅375px相当）をブラウザ内に再現し、大きいカードのタイトルが一行で表示されること（`white-space`計算・`scrollWidth`比較で確認）、小さいバッジも新配色でテキストの欠けがないこと（`scrollWidth <= clientWidth`を確認）をスクリーンショット・数値の両方で確認。**未実施**: 実機（実際のiPhone）での最終見た目確認はユーザー対応待ち
